...
for s in [3, 6, 12]:
    ...
    temp1 = np.transpose([ZDiscExp(s,b,asimov_only=False, CLDiscbool=False) for b in b_arraya])

    #Computing CCGV Z
    temp2 = [ZDiscExpCCGV(s,b) for b in b_arraya]
...
for s in [3, 6, 12]:
    #Computing Asimov Z, Mean Z, and Median Z
    temp1 = np.transpose([ZExclExp(s,b,asimov_only=False, CLExclbool=False) for b in b_arrayb])

    #Computing KM Z
    temp2 = [ZExclExpKM(s,b) for b in b_arrayb]
