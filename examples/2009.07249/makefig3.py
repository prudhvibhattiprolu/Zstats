...
for sbyb in [2, 10, 100]:
    ...
    temp = np.transpose([ZDiscExp(s,s/sbyb,dbbyb*s/sbyb,asimov_only=False, CLDiscbool=False) for s in s_array])
...
for sbyb in [5, 0.5]:
    #Calculate Asimov Z, Mean Z, Median Z for fixed s/bhat
    temp = np.transpose([ZExclExp(s,s/sbyb,dbbyb*s/sbyb,asimov_only=False, CLExclbool=False) for s in s_array])
