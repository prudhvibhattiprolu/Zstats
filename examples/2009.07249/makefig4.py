...
for sbyb in [1, 10, 100]:
    ...
    temp1 = np.transpose([ZDiscExp(s,s/sbyb, CLDiscbool=False) for s in s_array])

    #fractional background uncertainty is 0.2
    temp2 = np.transpose([ZDiscExp(s,s/sbyb,0.2*s/sbyb, CLDiscbool=False) for s in s_array])

    #fractional background uncertainty is 0.5
    temp3 = np.transpose([ZDiscExp(s,s/sbyb,0.5*s/sbyb, CLDiscbool=False) for s in s_array])
...
for sbyb in [5, 0.5]:
    ...
    #fractional background uncertainty is 0
    temp1 = np.transpose([ZExclExp(s,s/sbyb, CLExclbool=False) for s in s_array])

    #fractional background uncertainty is 0.2
    temp2 = np.transpose([ZExclExp(s,s/sbyb,0.2*s/sbyb, CLExclbool=False) for s in s_array])

    #fractional background uncertainty is 0.5
    temp3 = np.transpose([ZExclExp(s,s/sbyb,0.5*s/sbyb, CLExclbool=False) for s in s_array])
