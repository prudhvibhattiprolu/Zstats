...
for sbyb in [1, 10, 100]:
    ...
    np.savetxt('fig3_disc_sbyb%s.dat' %(sbyb),np.transpose([s_array, temp1, temp2, temp3]),delimiter='\t',header='s \t dbhat/bhat=0 \t dbhat/bhat=0.2 \t dbhat/bhat=0.5',comments='#Fig3: The exact Asimov expected significance for discovery, when s/bhat=%s\n#' %(sbyb))
...
for sbyb in [5, 0.5]:
    ...
    #Printing data to a *.dat file
    np.savetxt('fig3_excl_sbyb%s.dat' %(sbyb),np.transpose([s_array, temp1, temp2, temp3]),delimiter='\t',header='s \t dbhat/bhat=0 \t dbhat/bhat=0.2 \t dbhat/bhat=0.5',comments='#Fig3: The exact Asimov expected significance for exclusion, when s/bhat=%s\n#' %(sbyb))
